...
class LoginController extends CommonController
{
    ...
    {
        if (!request()->isPost()) {
            return apiReturn(config('app_show_error'), '您没有权限', '', 403);
        }
        ...
        if (!$validate->check($param, 'Login.save')) {
            return apiReturn(config('app_show_error'), $validate->getError(), '', 400);
        }

        $bool = $this->checkLoginVerify($param['verify'], $param['sno'], $param['cardno']);
        if (false == $bool) {
            return apiReturn(config('app_show_error'), '非法请求', '', 400);
        }
        ...
        if ($id || $info) {
            ...
            return apiReturn(config('app_show_success'), '登录成功', $result, 200);
        } else {
            return apiReturn(config('app_show_error'), '登录失败', '', 200);
        }
    }
